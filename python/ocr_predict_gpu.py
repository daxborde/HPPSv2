...
def detection(img_path, detection_model, recognition_model, label_dict, is_video=False):
    if is_video:
        bgr_image = img_path
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        try:
            ...
            logging.error("One or more arguments missing. Make sure you've supplied a valid image directory and database path.")
            sys.exit()
    ...
    vis_save_dir = os.path.join(img_dir, "ocr_output", "")
    os.makedirs(vis_save_dir, exist_ok = True)

    print("Saving OCR annotations to {}...".format(vis_save_dir))
